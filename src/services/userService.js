...
import {
  ...
  updateProfile,
  signOut // ✅✅✅ FIX: signOut 함수를 firebase/auth에서 임포트해야 합니다. ✅✅✅
} from "firebase/auth";
...
// --- 이 authReadyPromise 헬퍼 함수와 그 안에 있던 onAuthStateChanged 리스너는 userStore.js에 있어야 합니다. ---
/*
...
// --- FIX: logoutUser 함수 정의 추가 ---
const logoutUser = async () => {
 try {
  console.log("[userService] 로그아웃 시도...");
  await signOut(auth); // ✅ 임포트된 signOut 함수와 auth 인스턴스 사용
  console.log("[userService] 로그아웃 성공.");
 } catch (error) {
  console.error("[userService] 로그아웃 오류:", error);
  // 사용자에게 오류 알림은 호출하는 쪽에서 처리할 수도 있습니다.
...
export {
 ...
 uploadProfileImage,
 deleteStorageFile,
 logoutUser // ✅✅✅ FIX: logoutUser 함수를 export 목록에 추가합니다. ✅✅✅
