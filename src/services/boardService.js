...
import {
  collection, getDocs, query, orderBy, doc, getDoc, addDoc, deleteDoc,
...
const updateComment = async (postId, commentId, updatedText) => { // ✅ updatedText는 문자열을 기대합니다.
  ...
  // ✅ updatedText가 문자열인지 확인하는 방어 로직 추가 (선택 사항이지만 안전함)
  if (typeof updatedText !== 'string') {
    console.error("[boardService] updateComment: updatedText 인자가 문자열이 아닙니다.", updatedText);
    throw new Error("댓글 내용 형식이 올바르지 않습니다.");
  }


  if (!updatedText.trim()) { // 이제 updatedText는 문자열이므로 .trim() 사용 가능
  ...
  try {
    console.log(`[boardService] 댓글 ${commentId} 문서 updateDoc 시도. 데이터: { text: "${updatedText.trim()}" }`); // ✅ trim()은 여기서 사용
...
const deletePost = async (postId) => {
  ...
  try {
    ...
    // 작성자 본인인지 확인
    if (postData.authorId !== user.uid && !userStore.isCurrentUserAdmin) {
      console.warn("[boardService] 게시글 삭제: 작성자 불일치 및 관리자 아님."); // 경고 메시지도 변경 가능
      throw new Error("게시글 작성자 또는 관리자만 삭제할 수 있습니다.");
    }
