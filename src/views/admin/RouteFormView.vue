<template>
    <div class="route-form-section">
      ...
      <div v-if="selectedImageUrl" class="image-preview">
        ...
        <img
          ...
          @click="openImageViewer(selectedImageUrl)"
        />
        <div
          v-if="
            isEditing &&
      ...
      <div v-if="isEditing" class="pitch-management-section">
        ...
        <div v-if="showPitchForm" class="pitch-form-container">
          ...
          <div class="image-preview">
            <!-- ✅ 클릭 이벤트 추가: 피치 이미지 -->
            <img :src="selectedPitchImageUrl" alt="피치 이미지" @click="openImageViewer(selectedPitchImageUrl)" />
      ...
      <div v-if="formError" class="error-message">{{ formError.message }}</div>
       ...
       <!-- ✅ 이미지 뷰어 모달/오버레이 -->
      <div class="image-viewer-overlay" v-if="showImageViewer" @click="closeImageViewer">
        <div class="image-viewer-content" @click.stop> <!-- 클릭 이벤트 전파 방지 -->
          <img :src="currentImage" alt="확대 이미지" />
          <button class="close-viewer" @click="closeImageViewer">X</button>
        </div>
      </div>
  ...
  const openImageViewer = (imageUrl) => {
    if (imageUrl) { // 이미지가 있을 때만 뷰어 열기
      currentImage.value = imageUrl;
      showImageViewer.value = true;
       // 뷰어 열릴 때 스크롤 방지 (선택 사항)
...
.image-viewer-content {
  position: relative; /* 닫기 버튼의 기준 */
  /* max-width: 90%; /* 최대 너비 */ /* ✅ max-width/height를 img로 이동 */
  /* max-height: 90%; /* 최대 높이 */ /* ✅ max-width/height를 img로 이동 */
  display: flex;
  justify-content: center; /* 이미지를 가운데 정렬 */
  align-items: center;
   overflow: hidden; /* ✅ 컨테이너 내용이 넘치면 숨김 */
...
.image-viewer-content img {
  /* ✅ 이미지가 컨테이너 내부에 비율 유지하며 꽉 차게 (잘리지 않도록) */
  width: 100%; /* ✅ 원본 비율 유지 */
  height: 100%; /* ✅ 원본 비율 유지 */
  object-fit: contain; /* ✅ 비율 유지하며 컨테이너에 맞춤 (잘리지 않음) */
}
